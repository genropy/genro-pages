...
from genro_pages import __main__ as cli
...
def test_manual_serves_html_from_another_directory(tmp_path):
    ...
    process = subprocess.Popen(
        [sys.executable, "-m", "genro_pages", "manual", "--directory", str(manual),
...
def test_manual_missing_directory(tmp_path):
    result = subprocess.run(
        [sys.executable, "-m", "genro_pages", "manual", "--directory", str(tmp_path)],
        capture_output=True, text=True, timeout=10)
...
def test_existing_page_launch(monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["genropages", "--modules", "/client"])
    cli.main()
    configuration.assert_called_once_with(
        "/client", "/tmp/genro-pages-8000", rpc_http_method="WSK")
